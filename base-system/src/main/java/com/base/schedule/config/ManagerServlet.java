...
public class ManagerServlet extends HttpServlet {
    ...
    private static final String PAGE =
            "\t <body>\n" +
                    ...
                    "\t     <a  target=\"_blank\" href=\"http://git.oschina.net/uncode/uncode-schedule\">【uncode-schedule】</a>&nbsp;&nbsp;&nbsp;&nbsp;uncode-loglout\t\t" +
...
*/
            ...
            if (StringUtil.isNotEmpty(del)) {
                ...
                response.sendRedirect(request.getSession().getServletContext().getContextPath() + "/uncode/schedule");
            } else if (StringUtil.isNotEmpty(logout)) {
                request.getSession().removeAttribute(UNCODE_SESSION_KEY);
                response.setContentType("text/html;charset=UTF-8");
                PrintWriter out = response.getWriter();
                out.write(HEAD);
                out.write(SCRIPT_LOGIN);
                ...
                response.sendRedirect(request.getSession().getServletContext().getContextPath() + "/uncode/schedule");
            } else if (StringUtil.isNotEmpty(stop)) {
                TaskDefine taskDefine = new TaskDefine();
                ...
                ConsoleManager.updateScheduleTask(taskDefine);
                response.sendRedirect(request.getSession().getServletContext().getContextPath() + "/uncode/schedule");
            } else if (StringUtil.isNotEmpty(bean) && StringUtil.isNotEmpty(method)) {
                TaskDefine taskDefine = new TaskDefine();
                taskDefine.setTargetBean(bean);
                taskDefine.setTargetMethod(method);
                String before = request.getParameter("before");
                if (StringUtil.isNotBlank(before)) {
                ...
                if (StringUtil.isNotEmpty(extKeySuffix)) {
                    taskDefine.setExtKeySuffix(extKeySuffix);
                }
                if (StringUtil.isNotEmpty(cronExpression) || StringUtil.isNotEmpty(period) || null != taskDefine.getStartTime()) {
                ...
                if (servers != null) {
                    ...
                    for (int i = 0; i < tasks.size(); i++) {
                        ...
                        sbTask.append("<tr class=\"info\">")
                                ...
                                .append("<td>");
                        if (StringUtil.isBlank(taskDefine.getPercentage())) {
                            sbTask.append("-");
                        } else {
                        ...
                        if (taskDefine.isStop()) {
                            ...
                            if (StringUtil.isNotBlank(taskDefine.getExtKeySuffix())) {
                    ...
                    out.write(SCRIPT);
                    String pageStr = PAGE;
                    String paths = request.getSession().getServletContext().getContextPath();
                    pageStr = pageStr.replace("uncode-loglout", "<a href=\"" + paths + "/uncode/schedule?logout=true\">退出</a>");
                    out.write(String.format(pageStr, request.getSession().getServletContext().getContextPath() + "/uncode/schedule",
